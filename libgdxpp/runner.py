...
if path_config_godot_executable_name.exists():
    ...
    print_warning('.godot-path does not exist, falling back to "godot" as the executable name/path...')

path_godot_executable = which(godot_executable_name)
if not path_godot_executable:
    print_fatal(
        'Unable to find the godot executable, is .godot-path an invalid path or does it not exist?',
        exit_code=2
    )
